	...
	Character(graphicsManager, dt, id, spriteScale, position, bodySize, pathToTexture, textureName, speed, hp)
{
	...
	// Inicializa a textura/animação de ataque do player
	if (this->numPlayer == 1)
...
{
	// Inicializa a animação de ataque e cria a hitbox da espada
	if (this->attacking) {
		...
		// Quando o ataque acabar, aplica de volta a animação normal e deleta a espada
...
void Player::updateAttackingAnimation()
{
	...
	// Quando a animação de ataque acabar (chegar na última posição da textura), acaba o ataque
