...
CollisionsManager::CollisionsManager(EntityList* entityList, Entity** platformList, Player* player1, Player* player2)
{
...
// Verifica as colisões e realiza as ações necessárias caso elas aconteçam
			...
			if (this->player1) {
				...
				if (entityList->operator[](k)->getId() == DARKKNIGHT) {
					...
					if (PlayingState::twoPlayers && this->player2) {
						...
						if (static_cast<DarkKnight*>(entityList->operator[](k))->getSwordHitbox() && static_cast<DarkKnight*>(entityList->operator[](k))->getSwordHitbox()->getShape()->getGlobalBounds().intersects(this->player2->getShape()->getGlobalBounds())) {
							this->player2->loseHp(200);
						}
					}
